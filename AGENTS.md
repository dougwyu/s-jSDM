...
- End-to-end R-level benchmark (2026-08-22, `Code/bench_realdata.R`, simulated communities, logit link, df=5, sampling=400, step_size=10, 30 epochs, CPU): 10000x200 — torch 218.2s vs Mojo 198.1s (~1.10x); 3000x60 — torch 13.8s vs Mojo 16.6s (Mojo loses; reticulate per-batch call overhead dominates at this size). Loss trajectories agree to max abs diff 0.02 over 30 epochs (rel ~1e-4), i.e. statistically equivalent but NOT bitwise identical through the full R path, unlike the Python-level same-noise parity checks. Net: the R-end-to-end Mojo advantage is much smaller than kernel/Python-loop benchmarks suggest; more realistic-dataset validation needed before any default flip.
