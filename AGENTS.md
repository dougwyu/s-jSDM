...
- End-to-end R-level benchmark (2026-08-22, `Code/bench_realdata.R`, simulated communities, logit link, df=5, sampling=400, step_size=10, 30 epochs, CPU): with seed transport + SIMD transcendentals and the auto backend default (`torch` mode now sets `SJSDM_MOJO_BACKEND=0`; `auto` leaves it unset), re-run at seed 91: 10000x200 — torch 146.5s vs auto(Mojo) 78.2s (~1.87x); 3000x60 — torch 12.8s vs auto 9.6s (~1.34x). Both workloads are now wins through the full R path; loss trajectories agree to ~1e-3 relative (statistically equivalent). Earlier noise-transport-era results (1.10x big, small losing) are superseded.
...
- Backend default (2026-08-22): `SJSDM_MOJO_BACKEND` is now auto-enabled — unset/"auto" uses the Mojo MC loss whenever the prebuilt server binary exists and the fit runs on CPU float32 logit/probit; `=1` forces it on (error if the binary is missing), `=0` forces the PyTorch path. In auto mode, batches with NaN responses dispatch to the masked PyTorch loss at runtime; `se()` still always uses the PyTorch loss via `allow_mojo=False`. Note: the dispatcher closure must be able to see `torch_tmp`, so the torch loss is defined before the gate in `_build_loss_function`.
- Real-data validation of the auto default (2026-08-22, `Code/bench_real_occurrence.R`, bundled datasets, logit link, df=5, sampling=400, step_size=10, 30 epochs, CPU): butterflies (2609 sites x 55 real species) — torch 11.0s vs auto 8.4s (~1.31x), final losses agree to 0.03 (12.409 vs 12.440); auto default validated on realistic data. Eucalypts (458x12) exposed a pre-existing training instability unrelated to the backend: at learning_rate=0.1 the fit sits on a stability boundary and one deterministic trajectory diverges to a degenerate plateau (~38.8); at lr=0.05 BOTH backends diverge mid-run. Divergence is reproducible per backend because Model_sjSDM uses its own fixed init seed (42), so R's set.seed does not change the trajectory.
