...
- Transport-overhead profile (2026-08-22, `Code/profile_transport.R`): reticulate is NOT the bottleneck — `fit()` is a single Python call; R-side/dispatch residual is only 3-6% of wall time. The gap lives entirely in per-loss-call time: at batch_size=10 (step_size), Mojo costs 0.8->1.6 ms/call at 3000x60 and 2.4->4.5 ms/call at 10000x200 vs torch. Direct Python measurement reproduces these numbers with zero kernel work (1.44 ms/call at sp=60, 4.61 ms/call at sp=200), i.e. the calls are pure fixed overhead dominated by writing the noise tensor through the worker pipe (400x10x200 float32 = 3.2 MB/request at sp=200). Candidate fixes: ship an RNG seed instead of the noise tensor (parity through R is already statistical-only, so exact-stream matching is no longer required), use shared memory for the noise array, or restrict the Mojo backend to large-batch configurations where compute dominates.
