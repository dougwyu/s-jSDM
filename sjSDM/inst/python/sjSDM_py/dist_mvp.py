...
def MVP_logLik(Y, X, sigma, device, dtype, batch_size=25, alpha = 1.0, sampling=1000, link="probit", individual=False, theta = None, noise = None):
    ...
    data = torch.utils.data.TensorDataset(torch.tensor(Y, dtype=dtype, device=torch.device('cpu')), 
                                          torch.tensor(X, dtype=dtype, device=torch.device('cpu')),
                                          torch.arange(X.shape[0], dtype=torch.long, device=torch.device('cpu')))
    ...
    logLik = []
    for step, (y, pred, indices) in enumerate(DataLoader):
