...
Python environment, and build the Mojo worker:
...
embed absolute paths, so redo these two commands after moving or
...
> error, just no acceleration. If you prefer a normal install, set
> `SJSDM_MOJO_SERVER_BIN` to the absolute path of
> `work/mojo-backend/mc_grad_server_bin` before fitting.
